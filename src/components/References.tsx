        ...
        {references.map((reference, index) => (
          <Popover key={index}>
            <PopoverTrigger asChild>
              <button className="reference-card bg-gray-800 rounded-md border border-cyan-500/20 p-2 sm:p-3 lg:p-2 xl:p-2 2xl:p-2 hover:border-cyan-500/40 transition-all duration-300 cursor-pointer hover:bg-gray-750 w-full text-left h-full flex flex-col justify-center">
                <div className="space-y-0">
                  <h5 className="text-lg sm:text-xl lg:text-base xl:text-lg 2xl:text-lg font-semibold text-white leading-tight">{reference.name}</h5>
                  <h6 className="text-cyan-300 font-medium text-base sm:text-lg lg:text-sm xl:text-base 2xl:text-sm leading-tight">{reference.title}</h6>
                </div>
              </button>
            </PopoverTrigger>
            <PopoverContent 
