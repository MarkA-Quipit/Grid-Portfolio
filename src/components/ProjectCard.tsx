...
const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  if (isTechShowcase) {
    ...
    return (
      <TooltipProvider>
        <div className={`project-card ${size} rounded-lg border border-cyan-500 border-opacity-40 p-6 flex items-center justify-center relative overflow-hidden transition-all duration-300 shadow-[0_0_15px_rgba(6,182,212,0.2)] h-full hover:-translate-y-0.5 hover:shadow-[0_0_25px_rgba(6,182,212,0.4)] hover:border-opacity-80 md:p-4 md:rounded-md`}>
          <div className="card-content relative z-[2] text-white w-full text-center">
            <h3 className="card-title font-bold mb-6 leading-tight text-3xl md:text-xl md:mb-4">{title}</h3>
            {description && (
              <p className="card-description leading-relaxed opacity-90 text-lg mb-8 md:text-base md:mb-6">{description}</p>
            )}
            
            {/* Technology logos in horizontal line - centered and responsive */}
            <div className="flex items-center justify-center gap-4 w-full flex-wrap">
              {technologies.map((tech, index) => (
                <Tooltip key={index}>
                  <TooltipTrigger asChild>
                    <div className="w-14 h-14 rounded-lg bg-white/10 border border-white/20 hover:scale-110 hover:bg-white/20 transition-all duration-200 cursor-pointer flex items-center justify-center flex-shrink-0 backdrop-blur-sm md:w-12 md:h-12">
                      <img
                        src={tech.icon}
                        alt={tech.name}
                        className="w-8 h-8 object-contain md:w-7 md:h-7"
                        onError={(e) => {
                          // Fallback to a generic icon if the image fails to load
                          e.currentTarget.style.display = 'none';
                          e.currentTarget.parentElement!.innerHTML = `<span class="text-sm font-bold text-white md:text-xs">${tech.name.charAt(0)}</span>`;
                        }}
                      />
                    </div>
                  </TooltipTrigger>
